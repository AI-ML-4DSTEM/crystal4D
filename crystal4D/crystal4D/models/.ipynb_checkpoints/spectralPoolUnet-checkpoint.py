...
def spectralPoolUnet(input_channel, image_size, filter_size, n_depth, dp_rate = 0.1, act = 'relu'):
    ...
    for encode in range(int(np.log2(image_size))-1):
        if image_size//(2**encode) > 8:
            conv = convSpectral2d_block(pool, np.minimum(filter_size * (2**encode), 256), n_depth=n_depth, dp_rate=dp_rate,
                                        activation=act)
            pool = poolSpectral.SpectralMaxPool2D(img_size = image_size//(2**encode))(conv)
            skips.append(conv)
